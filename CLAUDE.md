...
- 技術面角度會明確要求 RSI、MACD、布林通道、成交量趨勢、波浪理論五項指標（結果放在 `technical_indicators` 欄位），
  不是只給支撐/阻力價位。
- 2026-08-26 新增：技術面角度加入第五項指標「波浪理論（Elliott Wave）」，加入前先上網查證過方法論
  （三條硬性鐵律：第2浪不可跌破第1浪起點、第3浪不可是第1/3/5浪中最短的一浪、第4浪不可與第1浪價格重疊；
  其餘費波那契比例只是guideline不是鐵律；學術上Aronson、Batchelor & Ramyar都質疑其可預測性與主觀性）。
  落地方式：要求回報時附上「目前浪位/級別＋失效價位（invalidation level）＋替代計數（若有）」，且強調
  只能當輔助/確認角度、須跟其他四項指標交叉比對，不能單獨作為進場依據；Verify 階段加入「波浪計數違反鐵律」
  的檢查（可客觀驗證對錯，發現時列入 agy_errors 而非 conflicting_facts）。
- 2026-08-14 新增：Synthesize 階段除了原本保守的「進場策略框架」小節，另外並列一個獨立的
  「動能策略觀點（順勢／動能型，對照用）」小節，給採動能/順勢交易風格、不要求四角度全部印證、
  不等結構性突破確認的讀者對照參考。兩套框架判準不同、可能給出不同方向的建議，筆記中會分開寫、
  不混寫成同一套結論，並在動能小節結尾提醒 whipsaw（假訊號連續停損）風險。
- 2026-08-14 再次調整：發現「進場策略框架」小節其實一直都有寫偏多/偏空各自的具體觸發價位、
  停損、目標（見 research/taiex-futures.md、jpy-futures.md），但回傳給使用者看的最終摘要
  （Synthesize 步驟4）之前只講方向跟建不建議進場，沒把這些具體數字帶出來，導致使用者要自己回去
  翻整份筆記才找得到。已調整步驟4的指示，要求最終摘要必須把偏多/偏空兩個方向的觸發條件各濃縮成
  一句話帶出來，不要停在「不建議進場」就結束。
- 2026-08-14 第三次調整：使用者詢問「其他投資策略的建議」，選擇固定加進workflow（比照動能小節）。
  依 `research/book/18-investment-strategies-taxonomy.md` 的16種策略逐一檢視哪些真的適用於
  「單一標的期貨/外匯即時研究」這個場景（多數股票選股風格如價值/成長/品質/GARP/股息/多因子、
  以及產業輪動/配對交易/市場中性/算法交易/系統化量化都不適用——這些需要跨標的比較、選股池、
  或程式化執行基礎設施，不是這個workflow研究單一標的的場景），新增兩個對照小節：
  「反轉／逆勢策略觀點」（跟動能小節鏡像對照，判斷現價是否處於統計極端、若否明講不適用）與
  「利差交易觀點」（僅適用外匯/貨幣期貨類標的，非此類標的直接寫「不適用」，不勉強套用）。
  全球總體策略與選擇權收益策略評估後**沒有**加入：全球總體策略跟既有「總體/政策面」角度的實質
  差異不夠大（只是換個包裝重講一次總體面讀到的東西），選擇權收益策略則因為這個workflow目前沒有
  蒐集隱含波動率/選擇權權利金資料，硬寫會流於空泛的通用建議、不夠具體可操作，故先不加，等以後
  真的要蒐集這類資料再考慮補上。
